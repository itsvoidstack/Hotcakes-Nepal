...
export async function sendVacancyNotificationEmail(options: VacancyEmailOptions): Promise<{ success: boolean; message: string; error?: string }> {
  ...
  if (!mailTransporter) {
    console.warn('⚠️ Missing SMTP credentials (SMTP_USER/SMTP_PASS) in environment variables.');
    return {
      success: false,
      message: `Destination is ${recipient}, but SMTP credentials (SMTP_USER, SMTP_PASS) are missing in environment variables.`,
      error: 'Missing SMTP credentials in environment variables.',
    };
  }

  try {
    const smtpFrom = process.env.SMTP_FROM || `"Hotcakes Nepal" <${process.env.SMTP_USER}>`;
    ...
    return {
      success: true,
      message: `Email notification sent successfully via Nodemailer to ${recipient} (Message ID: ${info.messageId})`,
    };
  } catch (err: unknown) {
          ...
          ${
            isTest
              ? `<div style="background-color: #F0FDF4; border: 1px solid #BBF7D0; padding: 14px; border-radius: 8px; margin-bottom: 20px; color: #166534; font-weight: bold; font-size: 14px;">
                  ✅ Test Email Verified! Gmail SMTP notification system is working correctly.
                </div>`
